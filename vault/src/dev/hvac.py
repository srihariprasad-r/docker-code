...
class hvacClient(object):
    ...
    def encode_ssn_ccn(self, ssn, transform_mount_point, value):
        url = self.client.url + "/v1/" + transform_mount_point + "/encode/" + self.ssn_role if ssn else self.ccn_role
        payload = "{\n  \"value\": \"" + value + "\",\n  \"transformation\": \"" + self.ssn_role if ssn else self.ccn_role + "\"\n}"
        headers = {
            'X-Vault-Token': self.client.token,
            'X-Vault-Namespace': self.client.namespace,
    ...
    def decode_ssn_ccn(self, ssn, transform_mount_point, value):
        url = self.client.url + "/v1/" + transform_mount_point + "/decode/" + self.ssn_role if ssn else self.ccn_role
        payload = "{\n  \"value\": \"" + value + "\",\n  \"transformation\": \"" + self.ssn_role  if ssn else self.ccn_role + "\"\n}"
        headers = {
            'X-Vault-Token': self.client.token,
            'X-Vault-Namespace': self.client.namespace,
    ...
    def _encrypt_non_ssn_ccn(self, value, keyname, mountpath):
